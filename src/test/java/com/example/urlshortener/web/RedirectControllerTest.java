...
class RedirectControllerTest {
    @Test void returnsTemporaryNoStoreRedirect() {
        ...
        var response = new RedirectController(resolver).redirect("aZ3kP9mQ2x");
    ...
    @Test void rejectsNotFoundAndDependencyOutcomes() {
        ...
        assertEquals(404, assertThrows(com.example.urlshortener.web.error.ApiException.class, () -> new RedirectController(resolver).redirect("missingcode")).status().value());
        assertEquals(503, assertThrows(com.example.urlshortener.web.error.ApiException.class, () -> new RedirectController(resolver).redirect("aZ3kP9mQ2x")).status().value());
    }
}
